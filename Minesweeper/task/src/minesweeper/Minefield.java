...
public class Minefield {
    ...
    int numberOfColumns, numberOfRows, numberOfMines;
    Coordinates[][] field;
    ...
    public boolean coordinateCanBeFlagged(int columnIndex, int rowIndex) {
        Coordinates coordinate = this.field[columnIndex][rowIndex];
        return coordinate.getSymbol() == '.' || coordinate.getSymbol() == '*';
    }

    public void flagCoordinate(int columnIndex, int rowIndex) {
        Coordinates coordinate = this.field[columnIndex][rowIndex];
        coordinate.toggleFlag();
    }
    ...
    public boolean gameOver() {
        for (int i = 0; i < this.numberOfColumns; i++) {
            for (int j = 0; j < this.numberOfRows; j++) {
                ...
                if (emptyCellFlagged || mineNotFlagged) return false;
            }
        }
        return true;
    }
    ...
    private void initializeField() {
        this.field = new Coordinates[numberOfColumns][numberOfRows];
        for (int i = 0; i < numberOfColumns; i++) {
            for (int j = 0; j < numberOfRows; j++) {
                field[i][j] = new Coordinates();
            }
        }
    }
    ...
    private char findAdjacentMines(int columnIndex, int rowIndex) {
        int checkColumnIndex, checkRowIndex;
        ...
        for (int i = -1; i < 2; i++) {
            ...
            for (int j = -1; j < 2; j++) {
                ...
                if (!(checkColumnIndex == columnIndex && checkRowIndex == rowIndex) &&
                        coordinateInField(checkColumnIndex, checkRowIndex)) {
